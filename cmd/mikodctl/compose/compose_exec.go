...
import (
	...
	"github.com/localfont/mikodctl/v2/pkg/composer"
)
...
func execCommand() *cobra.Command {
	...
	cmd.Flags().BoolP("no-TTY", "T", !isTerminal, "Disable pseudo-TTY allocation. By default mikodctl compose exec allocates a TTY.")
...
func execAction(cmd *cobra.Command, args []string) error {
	...
	// https://github.com/localfont/mikodctl/blob/v1.0.0/cmd/mikodctl/exec.go#L116
	if interactive && detach {
		return errors.New("currently flag -i and -d cannot be specified together (FIXME)")
	}
	// https://github.com/localfont/mikodctl/blob/v1.0.0/cmd/mikodctl/exec.go#L122
	if !noTty && detach {
