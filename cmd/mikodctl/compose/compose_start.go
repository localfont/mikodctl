...
import (
	...
	"github.com/localfont/mikodctl/v2/pkg/cmd/compose"
...
func startAction(cmd *cobra.Command, args []string) error {
	...
	mikodctlCmd, mikodctlArgs := helpers.GlobalFlags(cmd)
	...
	for _, svc := range services {
		...
		if err := startContainers(ctx, client, containers, &globalOptions, mikodctlCmd, mikodctlArgs); err != nil {
			return err
		}
	}
...
func startContainers(ctx context.Context, client *containerd.Client, containers []containerd.Container, globalOptions *types.GlobalCommandOptions, mikodctlCmd string, mikodctlArgs []string) error {
	...
	for _, c := range containers {
		...
		eg.Go(func() error {
			...
			if err := containerutil.Start(ctx, c, false, false, client, "", "", (*config.Config)(globalOptions), mikodctlCmd, mikodctlArgs); err != nil {
				return err
			}
