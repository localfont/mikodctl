...
import (
	...
	"github.com/localfont/mikodctl/v2/pkg/testutil/testregistry"
)
...
`, testutil.NginxAlpineImage)
	...
	go func() {
		// unbuffer(1) emulates tty, which is required by `mikodctl run -t`.
...
services:
  svc0:
    ...
    x-mikodctl-cosign-public-key: %s
    x-mikodctl-sign: cosign
    x-mikodctl-cosign-private-key: %s
    entrypoint:
      - stty
  svc1:
    build: .
    image: %s
    x-mikodctl-verify: cosign
    x-mikodctl-cosign-public-key: dummy_pub_key
    x-mikodctl-sign: cosign
    x-mikodctl-cosign-private-key: %s
    entrypoint:
      - stty
  svc2:
    build: .
    image: %s
    x-mikodctl-verify: none
    x-mikodctl-sign: none
    entrypoint:
      - stty
