...
// TestUnknownCommand tests https://github.com/localfont/mikodctl/issues/487
...
// TestMikodctlConfig validates the configuration precedence [CLI, Env, TOML, Default] and broken config rejection
func TestMikodctlConfig(t *testing.T) {
	testCase := nerdtest.Setup()

	// Docker does not support mikodctl.toml obviously
	...
	testCase.SubTests = []*test.Case{
		...
		},
		{
			...
			Config:      test.WithConfig(nerdtest.MikodctlToml, `snapshotter = "dummy-snapshotter-via-toml"`),
		},
		{
		...
		},
		{
			...
			Command:     test.Command("info"),
			Expected:    test.Expects(1, []error{errors.New("failed to load mikodctl config")}, nil),
			Config: test.WithConfig(nerdtest.MikodctlToml, `# containerd config, not mikodctl config
version = 2`),
		},
	}
