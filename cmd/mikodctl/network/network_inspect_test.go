...
func TestNetworkInspectBasic(t *testing.T) {
	...
	testCase.SubTests = []*test.Case{
		...
		{
			Description: "none",
			Require:     nerdtest.MikodctlNeedsFixing("no issue opened"),
		...
		},
		{
			...
			Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
				return &test.Expected{
					ExitCode: 0,
					Output: func(stdout string, t tig.T) {
						// Note: some functions need to be tested without the automatic --namespace mikodctl-test argument, so we need
	...
	testCase.SubTests = []*test.Case{
		{
			...
			Setup: func(data test.Data, helpers test.Helpers) {
				...
				// See https://github.com/localfont/mikodctl/issues/4322
		...
		{
			Description: "Display containers belonging to multiple networks in the output of mikodctl network inspect",
			Setup: func(data test.Data, helpers test.Helpers) {
