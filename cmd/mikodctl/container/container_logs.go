...
func LogsCommand() *cobra.Command {
	const shortUsage = "Fetch the logs of a container. Expected to be used with 'mikodctl run -d'."
	const longUsage = `Fetch the logs of a container.

The following containers are supported:
- Containers created with 'mikodctl run -d'. The log is currently empty for containers created without '-d'.
- Containers created with 'mikodctl compose'.
