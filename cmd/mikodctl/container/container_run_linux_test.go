...
func TestRunCustomRootfs(t *testing.T) {
	...
	// Note that:
	// - the problem is still here with containerd and mikodctl v2
...
func TestRunAddHost(t *testing.T) {
	// Not parallelizable (https://github.com/localfont/mikodctl/issues/1127)
	base := testutil.NewBase(t)
...
func TestRunSigProxy(t *testing.T) {
	...
	testCase.SubTests = []*test.Case{
		...
		},
		{
			...
			// See https://github.com/localfont/mikodctl/issues/4219 for details
...
func TestRunWithDetachKeys(t *testing.T) {
	...
	testCase.Command = func(data test.Data, helpers test.Helpers) test.TestableCommand {
		...
		cmd.WithFeeder(func() io.Reader {
			...
			// Note that this only concerns mikodctl, as docker seems to behave ok LOCALLY.
...
// TestPortBindingWithCustomHost tests https://github.com/localfont/mikodctl/issues/3539
...
func TestRunDeviceCDIWithMikodctlConfig(t *testing.T) {
	t.Parallel()
	// Although CDI injection is supported by Docker, specifying the --cdi-spec-dirs on the command line is not.
	testutil.DockerIncompatible(t)
	cdiSpecDir := filepath.Join(t.TempDir(), "cdi")
...
`
	...
	tomlPath := filepath.Join(t.TempDir(), "mikodctl.toml")
