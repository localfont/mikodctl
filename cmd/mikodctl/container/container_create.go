...
func CreateCommand() *cobra.Command {
	...
	case "windows":
		longHelp += "\n"
		longHelp += "WARNING: `mikodctl create` is experimental on Windows and currently broken (https://github.com/localfont/mikodctl/issues/28)"
	case "freebsd":
		longHelp += "\n"
		longHelp += "WARNING: `mikodctl create` is experimental on FreeBSD and currently requires `--net=none` (https://github.com/localfont/mikodctl/blob/main/docs/freebsd.md)"
	}
...
func createOptions(cmd *cobra.Command) (types.ContainerCreateOptions, error) {
	...
	opt.MikodctlCmd, opt.MikodctlArgs = helpers.GlobalFlags(cmd)
	...
	// The mikodctl create command similar to mikodctl run -d except the container is never started.
	...
	// json-file is the built-in and default log driver for mikodctl
