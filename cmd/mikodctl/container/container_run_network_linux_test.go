...
func TestRunContainerWithStaticIP(t *testing.T) {
	...
	testCases := []struct {
		...
		// XXX see https://github.com/localfont/mikodctl/issues/3101
		// docker 24 silently ignored the ip - now, docker 26 is erroring out - furthermore, this ip only makes sense
		// in the context of mikodctl bridge network, so, this test needs rewritting either way
		/*
			{
				ip:                "10.4.0.2",
				shouldSuccess:     true,
...
dns_search = ["example.com", "test.local"]`
	...
	testCase := &test.Case{
		Config: test.WithConfig(nerdtest.MikodctlToml, configContent),
		...
		SubTests: []*test.Case{
			{
				...
				Command: func(data test.Data, helpers test.Helpers) test.TestableCommand {
					mikodctlTomlContent := string(helpers.Read(nerdtest.MikodctlToml))
					helpers.T().Log("NERDCTL_TOML file content:\n%s", mikodctlTomlContent)
...
// See https://github.com/localfont/mikodctl/pull/4526
