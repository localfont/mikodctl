...
import (
	...
	mikodctlcontainer "github.com/localfont/mikodctl/v2/pkg/cmd/container"
...
func updateContainer(ctx context.Context, client *containerd.Client, id string, opts updateResourceOptions, cmd *cobra.Command) (retErr error) {
	...
	if cmd.Flags().Changed("restart") && restart != "" {
		if err := mikodctlcontainer.UpdateContainerRestartPolicyLabel(ctx, client, container, restart); err != nil {
			return err
		}
	}
