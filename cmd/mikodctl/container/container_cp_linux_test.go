...
// Obviously, none of this is fully windows ready - obviously `mikodctl cp` itself is not either, so, ok for now.
const (
	...
	cpFolderName = "mikodctl-cp-test"

	// Since mikodctl cp must NOT obey container wd, but instead resolve paths against the root, we set this
	// explicitly to ensure we do the right thing wrt that.
	containerCwd = "/mikodctl/cp/test"
		...
		for _, tc := range testCases {
			...
			t.Run(testCase.description, func(t *testing.T) {
				...
				if copyToContainer {
					if fromStdin {
						if toStdout {
							mikodctlCmd := base.Cmd("cp", "-", "-")
							mikodctlCmd.Run()
							mikodctlCmd.Assert(testCase.expect)
						} else {
							sourceSpec = "-"
							f, err := os.Open(sourceFile)
							assert.NilError(t, err)
							mikodctlCmd := base.Cmd("cp", sourceSpec, containerRunning+":"+destinationSpec)
							mikodctlCmd.Stdin = f
					...
					mikodctlCmd := base.Cmd("cp", containerRunning+":"+sourceSpec, destinationSpec)
					if toStdout {
						out := mikodctlCmd.Out()
						mikodctlCmd.Assert(testCase.expect)
