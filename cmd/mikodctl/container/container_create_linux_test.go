...
import (
	...
	"github.com/localfont/mikodctl/v2/pkg/testutil/nerdtest"
...
func TestCreateWithMACAddress(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(testName, func(tt *testing.T) {
			...
			tearDown()
			tt.Cleanup(tearDown)
			// This is currently blocked by https://github.com/localfont/mikodctl/pull/3104
			...
			if !wantErr {
				...
				// https://github.com/localfont/mikodctl/actions/runs/11392051487/job/31697214002?pr=3535#step:7:271
				...
				// See https://github.com/localfont/mikodctl/issues/3101
				if nerdtest.IsDocker() &&
...
// TestIssue2993 tests https://github.com/localfont/mikodctl/issues/2993
func TestIssue2993(t *testing.T) {
	...
	testCase.SubTests = []*test.Case{
		{
			Description: "Issue #2993 - mikodctl no longer leaks containers and etchosts directories and files when container creation fails.",
		...
		{
			Description: "Issue #2993 - mikodctl no longer leaks containers and etchosts directories and files when containers are removed.",
			Setup: func(data test.Data, helpers test.Helpers) {
				dataRoot := data.Temp().Path()
...
func TestCreateFromOCIArchive(t *testing.T) {
	...
	const sentinel = "test-mikodctl-create-from-oci-archive"
	...
	base.Cmd("start", "--attach", containerName).AssertOutContains("test-mikodctl-create-from-oci-archive")
}
...
func TestUsernsMappingCreateCmd(t *testing.T) {
	...
	testCase := &test.Case{
		...
		NoParallel: true,
		Setup: func(data test.Data, helpers test.Helpers) {
			data.Labels().Set("validUserns", "mikodctltestuser")
