...
func RunCommand() *cobra.Command {
	...
	case "windows":
		longHelp += "\n"
		longHelp += "WARNING: `mikodctl run` is experimental on Windows and currently broken (https://github.com/localfont/mikodctl/issues/28)"
	case "freebsd":
		longHelp += "\n"
		longHelp += "WARNING: `mikodctl run` is experimental on FreeBSD and currently requires `--net=none` (https://github.com/localfont/mikodctl/blob/main/docs/freebsd.md)"
	}
...
func setCreateFlags(cmd *cobra.Command) {
	...
	// entrypoint StringArray is an internal implementation to support `mikodctl compose` entrypoint yaml filed with multiple strings
...
func runAction(cmd *cobra.Command, args []string) error {
	...
	// defer setting `mikodctl/error` label in case of error
	defer func() {
		if err != nil {
	...
	if err := healthcheck.CreateTimer(ctx, c, (*config.Config)(&createOpt.GOptions), createOpt.MikodctlCmd, createOpt.MikodctlArgs); err != nil {
