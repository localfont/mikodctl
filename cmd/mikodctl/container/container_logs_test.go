...
`
	...
	testCase.Require = nerdtest.IsFlaky("https://github.com/localfont/mikodctl/issues/4782")
	if runtime.GOOS == "windows" {
		testCase.Require = nerdtest.MikodctlNeedsFixing("https://github.com/localfont/mikodctl/issues/4237")
	}
...
// Tests whether `mikodctl logs` properly separates stdout/stderr output
...
func TestLogsWithoutNewlineOrEOF(t *testing.T) {
	...
	// FIXME: mikodctl behavior does not match docker - test disabled for mikodctl until we fix
	testCase.Require = require.All(
		require.Linux,
	)
...
func TestLogsWithDetails(t *testing.T) {
	...
	// https://github.com/localfont/mikodctl/issues/4237
	if runtime.GOOS == "windows" {
		testCase.Require = nerdtest.MikodctlNeedsFixing("https://github.com/localfont/mikodctl/issues/4237")
	}
...
func TestLogsFollowNoExtraneousLineFeed(t *testing.T) {
	testCase := nerdtest.Setup()
	// This test verifies that `mikodctl logs -f` does not add extraneous line feeds
	testCase.Require = require.Not(require.Windows)
