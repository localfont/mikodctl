...
// TestMultiPlatformBuildPushNoRun tests if the push succeeds in a situation where mikodctl builds
// a Dockerfile without RUN, COPY, etc commands. In such situation, BuildKit doesn't download the base image
// so mikodctl needs to ensure these blobs to be locally available.
