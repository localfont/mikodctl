			...
			Expected: func(data test.Data, helpers test.Helpers) *test.Expected {
				return &test.Expected{
					...
					Output: expect.All(func(stdout string, t tig.T) {
						...
						hcLabel := inspect.Config.Labels["mikodctl/healthcheck"]
						...
						result.Run(&test.Expected{
							...
							Output: func(stdout string, _ tig.T) {
								assert.Assert(t, !strings.Contains(stdout, containerID),
									"expected mikodctl healthcheck timer for container ID %s to be removed after container stop", containerID)
							},
						})
					},
				}
					...
					Output: func(stdout string, t tig.T) {
						...
						result.Run(&test.Expected{
							...
							Output: func(stdout string, _ tig.T) {
								...
								assert.Assert(t, !strings.Contains(stdout, containerID),
									"expected mikodctl healthcheck timer for container ID %s to be removed after container removal", containerID)
							},
						})
					},
				}
					...
					Output: expect.All(func(stdout string, t tig.T) {
						...
						result.Run(&test.Expected{
							...
							Output: func(stdout string, _ tig.T) {
								...
								assert.Assert(t, strings.Contains(stdout, containerID),
									"expected to find mikodctl healthcheck timer containing container ID: %s", containerID)
							},
						})
