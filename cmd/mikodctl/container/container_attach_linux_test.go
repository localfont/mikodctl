...
/*
Important notes:
- for both docker and mikodctl, you can run+detach of a container and exit 0, while the container would actually fail starting
- mikodctl (not docker): on run, detach will race anything on stdin before the detach sequence from reaching the container
- mikodctl AND docker: on attach ^
- exit code variants: https://github.com/localfont/mikodctl/issues/3571
*/

func TestAttach(t *testing.T) {
	// In mikodctl the detach return code from the container after attach is 0, but in docker the return code is 1.
	// This behaviour is reported in https://github.com/localfont/mikodctl/issues/3571
	ex := 0
	...
	testCase.Command = func(data test.Data, helpers test.Helpers) test.TestableCommand {
		...
		cmd.WithFeeder(func() io.Reader {
			// Interestingly, and unlike with run, on attach, docker (like mikodctl) ALSO needs a pause so that the
...
// TestIssue3568 tests https://github.com/localfont/mikodctl/issues/3568
