...
CMD ["echo", "mikodctl-build-test-string"]`, testutil.CommonImage)
		...
		SubTests: []*test.Case{
			{
				...
				Expected: test.Expects(expect.ExitCodeSuccess, nil, expect.Equals("mikodctl-build-test-string\n")),
			},
			{
...
// This isn't currently supported by mikodctl with BuildKit OCI worker.
...
// TestBuildFromContainerd tests if an image can be built on an image pulled by mikodctl.
// This isn't currently supported by mikodctl with BuildKit OCI worker.
...
func TestBuildFromStdin(t *testing.T) {
	...
	dockerfile := fmt.Sprintf(`FROM %s
CMD ["echo", "mikodctl-build-test-stdin"]`, testutil.CommonImage)
...
func TestBuildWithDockerfile(t *testing.T) {
	...
	dockerfile := fmt.Sprintf(`FROM %s
CMD ["echo", "mikodctl-build-test-dockerfile"]
...
func TestBuildLocal(t *testing.T) {
	...
	const testFileName = "mikodctl-build-test"
	const testContent = "mikodctl"
...
func TestBuildWithIIDFile(t *testing.T) {
	...
	dockerfile := fmt.Sprintf(`FROM %s
CMD ["echo", "mikodctl-build-test-string"]
	`, testutil.CommonImage)
...
func TestBuildWithLabels(t *testing.T) {
	...
	dockerfile := fmt.Sprintf(`FROM %s
LABEL name=mikodctl-build-test-label
	`, testutil.CommonImage)
	...
	testCase := &test.Case{
		...
		Expected: test.Expects(expect.ExitCodeSuccess, nil, expect.Equals("{\"label\":\"test\",\"name\":\"mikodctl-build-test-label\"}\n")),
	}
...
CMD ["echo", "mikodctl-build-myorg/myapp"]`
		...
		SubTests: []*test.Case{
			{
				...
				Expected: test.Expects(0, nil, expect.Equals("mikodctl-build-test-string\n")),
			},
			{
