...
CMD ["echo", "mikodctl-test-builder-prune"]`, testutil.CommonImage)
...
CMD ["echo", "mikodctl-test-builder-prune"]`, testutil.CommonImage)
			...
			{
				Description: "Debug",
				// `mikodctl builder debug` is currently incompatible with `docker buildx debug`.
				Require:    require.All(require.Not(nerdtest.Docker)),
				NoParallel: true,
				Command: func(data test.Data, helpers test.Helpers) test.TestableCommand {
					dockerfile := fmt.Sprintf(`FROM %s
CMD ["echo", "mikodctl-builder-debug-test-string"]`, testutil.CommonImage)
					data.Temp().Save(dockerfile, "Dockerfile")
