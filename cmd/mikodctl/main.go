...
import (
	...
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/builder"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/checkpoint"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/completion"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/compose"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/container"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/helpers"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/image"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/inspect"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/internal"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/ipfs"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/login"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/manifest"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/namespace"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/network"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/search"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/system"
	"github.com/localfont/mikodctl/v2/cmd/mikodctl/volume"
	...
	"github.com/localfont/mikodctl/v2/pkg/version"
)
...
func initRootCmdFlags(rootCmd *cobra.Command, tomlPath string) (*pflag.FlagSet, error) {
	...
	if r, err := os.Open(tomlPath); err == nil {
		...
		if err := dec.Decode(cfg); err != nil {
			return nil, fmt.Errorf("failed to load mikodctl config (not daemon config) from %q (Hint: don't mix up daemon's `config.toml` with `mikodctl.toml`): %w", tomlPath, err)
		}
	...
	// -a is aliases (conflicts with mikodctl images -a)
	...
	// -n is aliases (conflicts with mikodctl logs -n)
	...
	rootCmd.PersistentFlags().String("data-root", cfg.DataRoot, "Root directory of persistent mikodctl state (managed by mikodctl, not by containerd)")
	...
	// Experimental enable experimental feature, see in https://github.com/localfont/mikodctl/blob/main/docs/experimental.md
	helpers.AddPersistentBoolFlag(rootCmd, "experimental", nil, nil, cfg.Experimental, "NERDCTL_EXPERIMENTAL", "Control experimental: https://github.com/localfont/mikodctl/blob/main/docs/experimental.md")
	...
	helpers.AddPersistentStringFlag(rootCmd, "bridge-ip", nil, nil, nil, aliasToBeInherited, cfg.BridgeIP, "NERDCTL_BRIDGE_IP", "IP address for the default mikodctl bridge network")
...
func newApp() (*cobra.Command, error) {
	tomlPath := ncdefaults.MikodctlTOML()
	if v, ok := os.LookupEnv("NERDCTL_TOML"); ok {
		tomlPath = v
	}

	short := "mikodctl is a command line interface for containerd"
	long := fmt.Sprintf(`%s
