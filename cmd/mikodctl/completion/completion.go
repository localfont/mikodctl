	...
	for netName, network := range netConfigs {
		if _, ok := excludeMap[netName]; !ok {
			...
			if network.MikodctlID != nil {
				candidates = append(candidates, *network.MikodctlID)
				candidates = append(candidates, (*network.MikodctlID)[0:12])
			}
		}
	}
...
func getVolumes(cmd *cobra.Command, globalOptions types.GlobalCommandOptions) (map[string]native.Volume, error) {
	...
	if err != nil {
		// The `mikodctl volume rm` does not have the flag `size`, so set it to false as the default value.
		volumeSize = false
	}
