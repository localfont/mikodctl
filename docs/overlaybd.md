...
| :zap: Requirement | mikodctl >= 0.15.0 |
| ----------------- | --------------- |
...
- Run `mikodctl` with `--snapshotter=overlaybd`
```console
mikodctl run --net host -it --rm --snapshotter=overlaybd registry.hub.docker.com/overlaybd/redis:6.2.1_obd
```
...
## Build OverlayBD image using `mikodctl image convert`

Nerdctl supports to convert an OCI image or docker format v2 image to OverlayBD image by using the `mikodctl image convert` command.

Before the conversion, you should have the `overlaybd-snapshotter` binary installed, which build from [accelerated-container-image](https://github.com/containerd/accelerated-container-image). You can run the command like `mikodctl image convert --overlaybd --oci <source_image> <target_image>` to convert the `<source_image>` to a OverlayBD image whose tag is `<target_image>`.
