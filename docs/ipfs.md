...
mikodctl automatically converts an image to IPFS-enabled when necessary.
For example, when mikodctl pushes an image to IPFS, if that image isn't an IPFS-enabled one, it converts that image to the IPFS-enabled one.
...
## Using mikodctl with IPFS

mikodctl supports an image name prefix `ipfs://` to handle images on IPFS.

### `mikodctl push ipfs://<image-name>`

For `mikodctl push`, you can specify `ipfs://` prefix for arbitrary image names stored in containerd.
When this prefix is specified, mikodctl pushes that image to IPFS.

```console
> mikodctl push ipfs://ubuntu:20.04
...
```console
> mikodctl push --estargz ipfs://fedora:36
...
### `mikodctl pull ipfs://<CID>` and `mikodctl run ipfs://<CID>`

You can pull an image from IPFS by specifying `ipfs://<CID>` where `CID` is the CID of the image.

```console
> mikodctl pull ipfs://bafkreicq4dg6nkef5ju422ptedcwfz6kcvpvvhuqeykfrwq5krazf3muze
...
`mikodctl run` also supports the same image name syntax.
When specified, this command pulls the image from IPFS.

```console
> mikodctl run --rm -it ipfs://bafkreicq4dg6nkef5ju422ptedcwfz6kcvpvvhuqeykfrwq5krazf3muze echo hello
hello
```
...
```
mikodctl tag ipfs://bafkreicq4dg6nkef5ju422ptedcwfz6kcvpvvhuqeykfrwq5krazf3muze ghcr.io/ktock/ubuntu:20.04-ipfs
mikodctl push ghcr.io/ktock/ubuntu:20.04-ipfs
```
...
### `mikodctl build` and `localhost:5050/ipfs/<CID>` image reference
...
:information_source: In the future version of mikodctl and BuildKit, `ipfs://` prefix should be supported in Dockerfile.
...
Make sure that `mikodctl ipfs registry serve` is running.
This allows `mikodctl build` to pull images from IPFS.

```
$ mikodctl ipfs registry serve &
```

Then you can build this Dockerfile using `mikodctl build`.

```console
> mikodctl build -t hello .
...
> mikodctl run --rm -it hello cat /hello
hello
```

> NOTE: `--ipfs` flag has been removed since v1.2.0. You need to launch the localhost registry by yourself using `mikodctl ipfs registry serve`.

#### Details about `localhost:5050/ipfs/<CID>` and `mikodctl ipfs registry`

As of now, BuildKit doesn't support `ipfs://` prefix so mikodctl achieves builds on IPFS by having a read-only local registry backed by IPFS.
...
This registry is provided as a subcommand `mikodctl ipfs registry`.
This command starts the registry backed by the IPFS repo of the current `$IPFS_PATH`
By default, mikodctl exposes the registry at `localhost:5050` (configurable via flags).

<details>
<summary>Creating systemd unit file for `mikodctl ipfs registry`</summary>

Optionally you can create systemd unit file of `mikodctl ipfs registry serve`.
An example systemd unit file for `mikodctl ipfs registry serve` can be the following.
`mikodctl ipfs registry serve` is aware of environment variables for configuring the behaviour (e.g. listening port) so you can use `EnvironmentFile` for configuring it.

```
[Unit]
Description=mikodctl ipfs registry serve

[Service]
EnvironmentFile-=/run/mikodctl-ipfs-registry-serve/env
ExecStart=mikodctl ipfs registry serve
...
```
mikodctl ipfs registry serve --listen-registry=localhost:5555
```

> NOTE: You'll also need to restart the registry when you change `$IPFS_PATH` to use.

> NOTE: `mikodctl ipfs registry [up|down]` has been removed since v1.2.0. You need to launch the localhost registry using `mikodctl ipfs registry serve` instead.

### Compose on IPFS

`mikodctl compose` supports same image name syntax to pull images from IPFS.
...
```
mikodctl compose up --build
```

```
mikodctl compose build
```
...
```console
> mikodctl image encrypt --recipient=jwe:mypubkey.pem ubuntu:20.04 ubuntu:20.04-encrypted
sha256:a5c57411f3d11bb058b584934def0710c6c5b5a4a2d7e9b78f5480ecfc450740
> mikodctl push ipfs://ubuntu:20.04-encrypted
...
```console
> mikodctl pull --unpack=false ipfs://bafkreifajsysbvhtgd7fdgrfesszexdq6v5zbj5y2jnjfwxdjyqws2s3s4
...
> mikodctl image decrypt --key=mykey.pem ipfs://bafkreifajsysbvhtgd7fdgrfesszexdq6v5zbj5y2jnjfwxdjyqws2s3s4 ubuntu:20.04-decrypted
sha256:b0ccaddb7e7e4e702420de126468eab263eb0f3c25abf0b957ce8adcd1e82105
> mikodctl run --rm -it ubuntu:20.04-decrypted echo hello
hello
```

## Running containers on IPFS with eStargz-based lazy pulling

mikodctl supports running eStargz images on IPFS with lazy pulling using Stargz Snapshotter.
...
```
mikodctl run --rm -it ipfs://bafkreibp2ncujcia663uum25ustwvmyoguxqyzjnxnlhebhsgk2zowscye echo hello
```
...
- See [`/docs/stargz.md`](./stargz.md) for details about the configuration of mikodctl for Stargz Snapshotter.
