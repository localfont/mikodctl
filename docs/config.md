# Configuring mikodctl with `mikodctl.toml`

| :zap: Requirement | mikodctl >= 0.16 |
|-------------------|-----------------|

This document describes the configuration file of mikodctl (`mikodctl.toml`).
This file is unrelated to the configuration file of containerd (`config.toml`) .

## File path
- Rootful mode:  `/etc/mikodctl/mikodctl.toml`
- Rootless mode: `~/.config/mikodctl/mikodctl.toml`
...
```toml
# This is an example of /etc/mikodctl/mikodctl.toml .
...
| `bridge_ip`         | `--bridge-ip`                      | `NERDCTL_BRIDGE_IP`       | IP address for the default mikodctl bridge network, e.g., 10.1.100.1/24                                                                                           | Since 2.0.1      |
...
2. Env var
3. TOML property
4. Built-in default value (Run `mikodctl --help` to see the default values)
