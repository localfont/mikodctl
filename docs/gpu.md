...
| :zap: Requirement | mikodctl >= 0.9 |
|-------------------|----------------|

> [!NOTE]
> The description in this section applies to mikodctl v2.3 or later.
> Users of prior releases of mikodctl should refer to <https://github.com/localfont/mikodctl/blob/v2.2.0/docs/gpu.md>

mikodctl provides docker-compatible NVIDIA and AMD GPU support.
...
## Options for `mikodctl run --gpus`

`mikodctl run --gpus` is compatible to [`docker run --gpus`](https://docs.docker.com/engine/reference/commandline/run/#access-an-nvidia-gpu).

You can specify number of GPUs to use via `--gpus` option.
The following examples expose all available GPUs to the container.

```
mikodctl run -it --rm --gpus all nvidia/cuda:12.3.1-base-ubuntu20.04 nvidia-smi
```

or

```
mikodctl run -it --rm --gpus=all rocm/rocm-terminal rocm-smi
```
...
```
mikodctl run -it --rm --gpus 'device=GPU-3a23c669-1f69-c64e-cf85-44e9b07e7a2a' nvidia/cuda:12.3.1-base-ubuntu20.04 nvidia-smi
```

Note that although `capabilities` options may be provided, these are ignored when processing the GPU request since mikodctl v2.3.

## Fields for `mikodctl compose`

`mikodctl compose` also supports GPUs following [compose-spec](https://github.com/compose-spec/compose-spec/blob/master/deploy.md#devices).
...
Available fields are the same as `mikodctl run --gpus`.
...
### `mikodctl run --gpus` fails due to an unresolvable CDI device

If the required CDI specifications for your GPU devices are not available on the
system, the `mikodctl run` command will fail with an error similar to: `CDI device injection failed: unresolvable CDI devices nvidia.com/gpu=all` (the
...
```
mikodctl run --device=nvidia.com/gpu=all
```
...
### `mikodctl run --gpus` fails when using the Nvidia gpu-operator

If the Nvidia driver is installed by the [gpu-operator](https://github.com/NVIDIA/gpu-operator).The `mikodctl run` will fail with the error message `(FATA[0000] exec: "nvidia-container-cli": executable file not found in $PATH)`.
...
And then, the `mikodctl run --gpus` can run successfully.
