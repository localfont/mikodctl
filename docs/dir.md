# mikodctl directory layout

## Config
**Default**: `/etc/mikodctl/mikodctl.toml` (rootful), `~/.config/mikodctl/mikodctl.toml` (rootless)

The configuration file of mikodctl. See [`config.md`](./config.md).
...
## Data
### `<DATAROOT>`
**Default**: `/var/lib/mikodctl` (rootful), `~/.local/share/mikodctl` (rootless)

Can be overridden with `mikodctl --data-root=<DATAROOT>` flag.

The directory is solely managed by mikodctl, not by containerd.
The directory has nothing to do with containerd data root `/var/lib/containerd`.

### `<DATAROOT>/<ADDRHASH>`
e.g. `/var/lib/mikodctl/1935db59`
...
e.g. `/var/lib/mikodctl/1935db59/containers/default/c4ed811cc361d26faffdee8d696ddbc45a9d93c571b5b3c54d3da01cb29caeb1`

Files:
- `resolv.conf`: mounted to the container as `/etc/resolv.conf`
- `hostname`: mounted to the container as `/etc/hostname`
- `log-config.json`: used for storing the `--log-opts` map of `mikodctl run`
- `<CID>-json.log`: used by `mikodctl logs`
...
e.g. `/var/lib/mikodctl/1935db59/names/default`
...
e.g. `/var/lib/mikodctl/1935db59/etchosts/default/c4ed811cc361d26faffdee8d696ddbc45a9d93c571b5b3c54d3da01cb29caeb1`
...
e.g. `/var/lib/mikodctl/1935db59/volumes/default/foo/_data`
...
Can be overridden with `mikodctl --cni-netconfpath=<NETCONFPATH>` flag and environment variable `$NETCONFPATH`.
...
Files:
- `mikodctl-<NWNAME>.conflist`: CNI conf list created by mikodctl
